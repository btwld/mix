...
### Border Radius (Legacy Convenience Methods)

Preferred style syntax: `borderRadius(.all(...))`, `borderRadius(.topLeft(...))`, etc.  
The method names below are legacy convenience APIs.
...
| `preferred` | Use `foregroundDecoration(.gradient(.linear(...)))` and `gradient(.linear(...))` for composable gradient styling. |
