---
title: "Box"
description: "Style Flutter containers with BoxStyler — decoration, padding, margins, transforms, constraints, and interaction variants in Mix."
---
...
| `elevation` | Sets Material Design elevation shadow. Values: `one`, `two`, `three`, `four`, `six`, `eight`, `nine`, `twelve`, `sixteen`, `twentyFour` |
...
| `onFocused` | Creates a variant for focused state (requires `PressableBox` or `Pressable` wrapper) |
| `onDisabled` | Creates a variant for disabled state |
| `onEnabled` | Creates a variant for enabled state (opposite of disabled) |

## Related

- [FlexBox](/documentation/widgets/flexbox) — flex layout with box styling
