---
title: "FlexBox"
description: "Build styled flex layouts in Flutter with FlexBoxStyler — direction, alignment, spacing, decoration, and variants for FlexBox, RowBox, and ColumnBox."
---
...
| `onFocused` | Creates a variant for focused state. Requires `Pressable` or `PressableBox` wrapper for focus tracking. |
| `onDisabled` | Creates a variant for disabled state |
| `onEnabled` | Creates a variant for enabled state (opposite of disabled) |

## Related

- [Box](/documentation/widgets/box) — `BoxStyler` API for standalone containers
