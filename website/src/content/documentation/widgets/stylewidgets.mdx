...
### `Box`
As already mentioned, the `Box` widget is like Flutter's `Container`, it can be styled with `BoxStyler`. See the [Box reference](/documentation/widgets/box) for the full API.
...
These are equivalent to the `Flex`, `Column` and `Row` widgets wrapped with a Container, they can be styled with `FlexBoxStyler`. See the [FlexBox reference](/documentation/widgets/flexbox) for the full API.
...
### `StyledText`
`StyledText` is equivalent to Flutter's `Text` widget, it can be styled with `TextStyler`. See the [StyledText reference](/documentation/widgets/text) for the full API.
...
### `StyledIcon`
`StyledIcon` is equivalent to Flutter's `Icon` widget, it can be styled with `IconStyler`. See the [StyledIcon reference](/documentation/widgets/icon) for the full API.
...
### `StyledImage`
`StyledImage` is equivalent to Flutter's `Image` widget, it can be styled with `ImageStyler`. See the [StyledImage reference](/documentation/widgets/image) for the full API.
...
icon(icon: Icons.star)  // Equivalent to StyledIcon(icon: Icons.star, style: icon)
```

This is useful for concise widget composition, especially when building reusable components.

### `Pressable` and `PressableBox`
This is equivalent to Flutter's **GestureDetector**, **MouseRegion**, and **Focus** widgets but with enhanced interaction state management and styling capabilities. See the [Pressable reference](/documentation/widgets/pressable) for the full API.
