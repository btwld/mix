...
### `Box`
As already mentioned, the `Box` widget is like Flutter's `Container`, it can be styled with `BoxStyler`. Learn more [here](/documentation/widgets/box).
...
These are equivalent to the `Flex`, `Column` and `Row` widgets wrapped with a Container, they can be styled with `FlexBoxStyler`. Learn more [here](/documentation/widgets/flexbox).
...
### `StyledText`
`StyledText` is equivalent to Flutter's `Text` widget, it can be styled with `TextStyler`. Learn more [here](/documentation/widgets/text).
...
### `StyledIcon`
`StyledIcon` is equivalent to Flutter's `Icon` widget, it can be styled with `IconStyler`. Learn more [here](/documentation/widgets/icon).
...
### `StyledImage`
`StyledImage` is equivalent to Flutter's `Image` widget, it can be styled with `ImageStyler`. Learn more [here](/documentation/widgets/image).
...
This is equivalent to Flutter's **GestureDetector**, **MouseRegion**, and **Focus** widgets but with enhanced interaction state management and styling capabilities. Learn more [here](/documentation/widgets/pressable).
