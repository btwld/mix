---
title: "Animations"
description: "Build implicit, phase, and keyframe animations with Mix styling APIs."
---
...
Mix provides duration extensions on `int` for convenience. These are available via `import 'package:mix/mix.dart'`:
...
**How to use:** Add `.animate(...)` to any `Style` or `Styler`. It works with regular state (e.g. `setState`) and with variants like `onHovered`, `onPressed`, and `onDark`.
...
final box = BoxStyler()
    ...
    .animate(.springDurationBased(1.s, bounce: 0.6));
...
Several tracks run in parallel to create one looping animation. The style builder applies scale, color, and opacity from the tracks.

For non-numeric types (like `Color`), provide a `tweenBuilder` (e.g., `ColorTween.new`) so the animation system knows how to interpolate between values. Numeric types (`double`, `int`) are interpolated automatically. The Keyframe animation feature allows you to use any custom Tween you want.
...
- [Dynamic Styling](/documentation/guides/dynamic-styling) — variant-triggered style changes
