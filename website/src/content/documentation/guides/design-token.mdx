---
title: Design Tokens
description: "Define, provide, and resolve Mix design tokens with MixScope."
---
...
final style = BoxStyler()
    .color($primary())  // Call the token with () to create a reference resolved at build time
    .size(100, 100);
```
...
| `TextStyleToken` | `TextStyle` | Typography styles. Use `.mix()` to get a `TextStyleMixRef` for `TextStyler.style()` |
...
`TextStyleToken` requires an extra `.mix()` call to produce a `TextStyleMixRef` that `TextStyler.style()` accepts:
...
Other token types (e.g. `ColorToken`, `SpaceToken`, `RadiusToken`) resolve directly with `()` — only `TextStyleToken` needs `.mix()`.
...
- [Styling with Mix](/documentation/guides/styling) — fluent Styler API patterns
- [Theming](/documentation/tutorials/theming) — build multi-theme apps with tokens
