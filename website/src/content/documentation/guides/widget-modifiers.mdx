...
Modifiers are applied in chain order: the first `.wrap()` becomes the innermost wrapper (closest to the widget), and the last `.wrap()` becomes the outermost wrapper:

```dart
// Widget tree (inside out): Box → Padding → Opacity → Align
final style = BoxStyler()
    .wrap(.padding(.all(8)))             // innermost — wraps the Box
    .wrap(.opacity(0.5))                 // middle
    .wrap(.align(alignment: .center));   // outermost — wraps everything
```
