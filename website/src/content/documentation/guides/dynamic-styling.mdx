...
## Creating Your Own Context Variants

The built-in variants cover common cases, but you can create custom `ContextVariant` instances that respond to any condition from `BuildContext` — such as an `InheritedWidget`, a feature flag, or app-specific state. See the [Creating a Custom Context Variant](/documentation/tutorials/creating-context-variants) tutorial for a step-by-step walkthrough.
