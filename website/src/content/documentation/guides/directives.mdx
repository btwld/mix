...
Directives can be chained. Number directives operate on `Prop<T>`, which requires the `.create()` constructor. This is the only case where you interact with `Prop` directly:
