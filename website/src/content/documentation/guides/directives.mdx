---
title: Directives
description: "Apply text and number directives to transform style values at resolution time."
---
...
final scaled = Prop.value(10.0).multiply(2).add(5).clamp(0, 30);
// Result: (10 * 2) + 5 = 25, clamped to [0, 30] = 25
```
...
// Use Prop.token() with number directives for token-based calculations
final scaledSize = Prop.token($baseSize).multiply(2);
final adjustedSize = Prop.token($baseSize).multiply(1.5);

// Clamped responsive values
final clampedPadding = Prop.value(16.0).clamp(8, 32);

// Chaining multiple directives
final computed = Prop.value(100.0).multiply(2).add(20).clamp(0, 300);
// Result: (100 * 2) + 20 = 220, clamped to [0, 300] = 220
```
...
- [Design Tokens](/documentation/guides/design-token) — token-based values with `MixScope`
