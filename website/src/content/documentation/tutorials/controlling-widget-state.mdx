...
If you want to control focus state (`onFocused`), you'll need to wrap the Box with a `Pressable` widget or use `PressableBox` instead. This is because tracking focus state requires a `Focus` widget, which `Pressable` provides:
...
class FocusedExample extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Now focus events will be tracked:
    return Pressable(child: style());
    // Alternatively, use:
