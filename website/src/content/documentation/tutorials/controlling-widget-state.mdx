...
description: "Manage hover, press, focus, and disabled states for Mix widgets."
---

# Controlling Widget State

This guide covers two approaches to managing widget states (hover, pressed, focused) in Mix: automatic state management and manual control with `WidgetStatesController`.

## Approach 1: Automatic State Management

Mix makes handling hover and press states effortless with any `StyledWidget`. These states are tracked for you automatically, requiring no extra setup:
...
class SimpleExample extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Just calling style() gives you a Box that handles these states for you.
    return style(); // returns a Box
  }
}
```
...
You do not need to wrap `Pressable` or do anything else for these states—they are tracked automatically.

---

If you want to control focus state (`onFocused`), use `PressableBox` instead of `Box`. This is because tracking focus state requires a `Focus` widget, which `PressableBox` provides:

```dart
class FocusedExample extends StatelessWidget {
  const FocusedExample({super.key});
  ...
  @override
  Widget build(BuildContext context) {
    return PressableBox(style: style, child: const SizedBox());
  }
}
```

In this example, the color changes to yellow when the widget has focus.
...
## Automatic State Tracking Scope

| State | Tracked automatically | Requires wrapper |
|-------|----------------------|------------------|
| `onHovered` | Yes — any `StyledWidget` | None |
| `onPressed` | Yes — any `StyledWidget` | None |
| `onFocused` | No | `Pressable` or `PressableBox` |
| `onDisabled` | No | `Pressable` or `PressableBox` (with `enabled: false`) |

## When to Use Each Approach

**Use automatic state management (Approach 1) when:**
- You need simple hover and press interactions (focus requires `PressableBox` or a `Pressable` wrapper)
