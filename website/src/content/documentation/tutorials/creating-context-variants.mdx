...
description: "Create and apply custom context variants driven by BuildContext conditions."
---
...
Start by defining a custom `InheritedWidget`:
...
BoxStyler()
    ...
    .variant(
      ContextVariant('custom_flag', (context) =>
        CustomInheritedWidget.of(context)?.flag ?? false,
      ),
      .color(Colors.blue),
    );
```
...
BoxStyler()
    .color(Colors.red)
    .size(100, 100)
    .onCustomFlag(.color(Colors.blue));
```
