...
@MixableSpec()
@immutable
final class ButtonSpec with _$ButtonSpec {
  @override
...
That's it — no manual `copyWith`, `lerp`, `props`, `==`, or `toString`. The generated mixin `_$ButtonSpec` carries all of them, along with `implements Spec<ButtonSpec>, Diagnosticable` for nominal subtyping and Flutter inspector integration.
...
mixin _$ButtonSpec implements Spec<ButtonSpec>, Diagnosticable {
  ...
  // ...toStringShort, toString({minLevel}), toDiagnosticsNode,
  //    debugFillProperties inlined by the generator
}
```
