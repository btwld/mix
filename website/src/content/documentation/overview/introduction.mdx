...
### Number Directives with `.create()`

For number directives like `.multiply()`, `.add()`, and `.clamp()`, use the `.create()` constructor:
...
final style = BoxStyler.create(
  ...
  height: Prop.value(100.0).clamp(50, 200),
);
```

For all other cases — including tokens — prefer the fluent API:
