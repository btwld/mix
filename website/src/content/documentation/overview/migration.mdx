...
The `$with` namespace is replaced with `.wrap*()` methods:

| v1.x Pattern | v2.0 Pattern |
|--------------|--------------|
| `$with.opacity(0.5)` | `.wrapOpacity(0.5)` |
| `$with.scale(2)` | `.wrapScale(2)` |
| `$with.rotate(45)` | `.wrapRotate(45)` |
...
// or with token method
BoxStyler().colorToken(primaryColor)
```
...
4. **Update modifiers**: Replace `$with.opacity()` with `.wrapOpacity()`
...
The operators have been redesigned to allow for concatenation and grouping, enabling the creation of conditions like, `(primary | secondary) & onHover`. For details, refer to the [variants guide](/documentation/guides/variants#combining-operators)
