...
<Callout type="warning">
  **Removed Utilities**: The global `$box`, `$text`, `$icon`, `$flex`, `$stack`, `$image`, `$flexbox`, and `$stackbox` accessors were removed in Mix 2.0. Use the corresponding Styler classes directly (e.g., `BoxStyler`, `TextStyler`).
</Callout>
...
| `Style($stack.alignment.center())` | `StackBoxStyler.alignment(.center)` |

#### Utility Prefixes Removed

The `$box`, `$text`, `$icon`, `$flex`, `$stack`, `$image`, `$flexbox`, and `$stackbox` utility prefixes were removed in Mix 2.0. Use the corresponding Styler class instead:
...
- **Callable Stylers**: Use `BoxStyler.<factory>(...)()` as shorthand for creating widgets
...
In our pursuit of API usability and efficiency, we introduced several replacements and removals across Mix versions. If you encounter issues not covered by this guide, please open an issue or send a PR.
...
The `Mix` class is now `Style`. Several methods in the renamed class, including `withVariants` and `withMaybeVariants`, were replaced or removed to boost performance and reduce potential confusion.
...
The `MixContextBuilder` was renamed to `SpecBuilder` to better reflect its purpose. Use `SpecBuilder` in current code.

#### Widget Updates

`TextMix` and `IconMix` widgets were replaced. Use `StyledText` and `StyledIcon` respectively.

#### Short Aliases

To enhance readability, short aliases and shorthand property/method names are replaced with their fully named counterparts.
...
```
### The use of $ (Removed in 2.0)

<Callout type="warning">
  **Note**: The `$` prefix utilities described below were **removed in Mix 2.0**. If you're migrating to Mix 2.0, use the Styler classes directly (e.g., `BoxStyler` instead of `$box`). See the [1.x to 2.0 migration section](#1x-to-20) above.
</Callout>
...
#### The Namespaces `$on` and `$with` (Removed in 2.0)

<Callout type="warning">
  **Removed**: In Mix 2.0, use the direct methods on Styler classes instead:
  - `$on.hover()` → `.onHovered()`
  - `$with.opacity()` → `.wrap(OpacityModifierMix())`
</Callout>
...
```dart
// v1.x syntax (removed in Mix 2.0)
PressableBox(
  onPress: () {},
  style: Style(
    $box.width(200),
