...
description: "Migrate existing Mix code from v1.x patterns to the Mix 2.0 Styler API."
---
...
The `$with` namespace is replaced with the `.wrap()` method or shorthand modifier methods:

| v1.x Pattern | v2.0 Pattern |
|--------------|--------------|
| `$with.opacity(0.5)` | `.wrap(.opacity(0.5))` |
...
- **Callable Stylers**: Any styler can be called directly to create its widget:

  ```dart
  final box = BoxStyler().color(Colors.blue);
  box(child: Text('Hello')); // creates a Box widget
  ```
...
4. **Update modifiers**: Replace `$with.opacity()` with `.wrap(.opacity(...))`
...
In our pursuit of API usability and efficiency, we introduced several deprecations in the latest Mix Package versions. If you encounter issues not covered by deprecation notices, please [open an issue](https://github.com/btwld/mix/issues).
...
| `Mix.withManyVariants` | Use `.variants([...])` or multiple `.variant()` calls on Styler classes |
