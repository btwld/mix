...
void main() {
  group('colorCodec', () {
    test('parses rgba() strings', () {
      final result = colorCodec.safeParse('rgba(59, 130, 246, 0.5)');
    ...
    test('parses rgba() strings with alpha 1', () {
      final result = colorCodec.safeParse('rgba(255, 255, 255, 1)');
    ...
    test('parses rgb() strings', () {
      final result = colorCodec.safeParse('rgb(59, 130, 246)');
    ...
    test('parses case-insensitive rgb()/rgba() strings', () {
      final rgbResult = colorCodec.safeParse('RGB(59, 130, 246)');
      final rgbaResult = colorCodec.safeParse('RGBA(59, 130, 246, 0.5)');
    ...
    test('parses flexible alpha numeric syntax currently supported', () {
      final leadingDot = colorCodec.safeParse('rgba(59, 130, 246, .5)');
      final trailingDot = colorCodec.safeParse('rgba(59, 130, 246, 1.)');
      final exponent = colorCodec.safeParse('rgba(59, 130, 246, 5e-1)');
    ...
    test('parses six-digit hex strings', () {
      final result = colorCodec.safeParse('#3B82F6');
    ...
    test('parses eight-digit CSS hex strings with trailing alpha', () {
      final result = colorCodec.safeParse('#3B82F680');
    ...
    test('rejects malformed function names', () {
      final result = colorCodec.safeParse('hsl(59, 130, 246)');

      expect(result.isFail, isTrue);
    });

    test('rejects wrong arity', () {
      final result = colorCodec.safeParse('rgba(59, 130, 246)');

      expect(result.isFail, isTrue);
    });

    test('rejects out-of-range rgb values', () {
      final result = colorCodec.safeParse('rgba(256, 130, 246, 0.5)');

      expect(result.isFail, isTrue);
    });

    test('rejects alpha outside 0..1', () {
      final result = colorCodec.safeParse('rgba(59, 130, 246, 1.5)');

      expect(result.isFail, isTrue);
    });

    test('rejects negative alpha values', () {
      final result = colorCodec.safeParse('rgba(59, 130, 246, -0.1)');

      expect(result.isFail, isTrue);
    });

    test('rejects non-finite alpha values', () {
      final result = colorCodec.safeParse('rgba(59, 130, 246, NaN)');

      expect(result.isFail, isTrue);
    });

    test('rejects unsupported legacy integer payloads', () {
      final result = colorCodec.safeParse(0xFF336699);
