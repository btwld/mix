...
void _expectDecodes<T extends Object>(JsonMap payload) {
  final result = mixProtocol.decodeStyle<T>({'v': 1, ...payload});
  expect(
    result,
    isA<MixProtocolSuccess<T>>(),
...
void main() {
  test('testing vocabulary exposes the built-in stylers', () {
    expect(
      SchemaStyler.values.map((value) => value.wireValue).toSet(),
      containsAll(<String>{'box', 'text', 'flex_box', 'icon', 'image'}),
    );
  });
...
Object? _encodedField(Object styler, String field) {
  final payload = switch (mixProtocol.encodeStyle(styler)) {
