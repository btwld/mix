...
void main() {
  ...
  test('text styler encodes supported text directives', () {
    final result = contract().encodeStyle(TextStyler.uppercase());
  ...
  test('text directives decode and re-encode symmetrically', () {
    ...
    final decoded = switch (contract().decodeStyle<TextStyler>(payload)) {
      MixProtocolSuccess<TextStyler>(:final value) => value,
      MixProtocolFailure<TextStyler>(:final errors) => fail('$errors'),
    };
    final encoded = switch (contract().encodeStyle(decoded)) {
      MixProtocolSuccess<JsonMap>(:final value) => value,
      MixProtocolFailure<JsonMap>(:final errors) => fail('$errors'),
    };
