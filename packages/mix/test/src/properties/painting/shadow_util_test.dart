  ...
  group('ElevationPropUtility', () {
    ...
    group('elevation getters', () {
      test('one creates elevation 1', () {
        ...
        expect(result.value.items.length, kElevationToShadow[1]!.length);
      });

      test('two creates elevation 2', () {
        final result = util.two;
        expect(result, isA<MockStyle<BoxShadowListMix>>());
        expect(result.value.items.length, kElevationToShadow[2]!.length);
      });

      test('three creates elevation 3', () {
        final result = util.three;
        expect(result, isA<MockStyle<BoxShadowListMix>>());
        expect(result.value.items.length, kElevationToShadow[3]!.length);
      });

      test('twentyFour creates elevation 24', () {
        final result = util.twentyFour;
        expect(result, isA<MockStyle<BoxShadowListMix>>());
        expect(result.value.items.length, kElevationToShadow[24]!.length);
      });
    });
    ...
    group('call method', () {
      test('creates elevation shadow for valid values', () {
        ...
        expect(result.value.items.length, kElevationToShadow[elevation]!.length);

        // Verify that the BoxShadowMix items contain the correct BoxShadow values
        final expectedShadows = kElevationToShadow[elevation]!;
        for (int i = 0; i < expectedShadows.length; i++) {
          final shadowMix = result.value.items[i];
          expect(shadowMix, isA<BoxShadowMix>());
          expect(shadowMix.resolve(MockBuildContext()), expectedShadows[i]);
        }
      });
    ...
    group('predefined elevations validation', () {
      ...
      test('elevation values match Flutter material design', () {
        ...
        expect(result.value.items.length, expectedShadows.length);
      });
    });
    ...
    group('shadow properties validation', () {
      test('elevation shadows have correct properties', () {
        ...
        for (int i = 0; i < expectedShadows.length; i++) {
          final mixProp = result.value.items[i];
    ...
    group('edge cases', () {
      test('handles minimum elevation', () {
        final result = util(1);
        expect(result.value.items.isNotEmpty, true);
      });
