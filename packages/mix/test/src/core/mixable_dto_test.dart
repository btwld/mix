  ...
  group('MixableDto', () {
    group('Factory constructors', () {
      test('value constructor creates _ValueMixDto', () {
        ...
        final dto = MixableDto.value(mockMix);

        expect(dto, isA<MixableDto<MockMix, String>>());
        expect(dto.isEmpty, false);
      });

      test('token constructor creates _TokenMixDto', () {
        const mockToken = MockToken('test_token');
        final dto = MixableDto.token(mockToken);
      ...
      test('maybeValue returns null for null input', () {
        final dto = MixableDto.maybeValue<MockMix, String>(null);
        expect(dto, isNull);
      });

      test('maybeValue returns MixableDto for non-null input', () {
        const mockMix = MockMix('test');
        final dto = MixableDto.maybeValue(mockMix);
    ...
    group('Resolve behavior', () {
      ...
      test('empty items list resolves to null', () {
        ...
        final dto = MixableDto.value(const MockMix('test'));
    ...
    group('Merge behavior', () {
      ...
      test('merging two values creates aggregated dto', () {
        ...
        final dto1 = MixableDto.value(mockMix1);
        final dto2 = MixableDto.value(mockMix2);

        final result = dto1.merge(dto2);
        expect(result, isA<MixableDto<MockMix, String>>());
      ...
      test('merging value with token creates aggregated dto', () {
        ...
        final valueDto = MixableDto.value(mockMix);
        final tokenDto = MixableDto.token(mockToken);
      ...
      test('merging aggregated dtos combines all items', () {
        ...
        final dto1 = MixableDto.value(mockMix1);
        final dto2 = MixableDto.value(mockMix2);
        final dto3 = MixableDto.value(mockMix3);

        final aggregated1 = dto1.merge(dto2);
        final aggregated2 = aggregated1.merge(dto3);

        expect(aggregated2, isA<MixableDto<MockMix, String>>());
    ...
    group('Smart grouping behavior', () {
      ...
      test('values after token are merged, but token remains separate', () {
        ...
        final tokenDto = MixableDto.token(mockToken);
        final valueDto1 = MixableDto.value(mockMix1);
        final valueDto2 = MixableDto.value(mockMix2);
    ...
    group('Equality and hashing', () {
      test('same values are equal', () {
        const mockMix = MockMix('test');
        final dto1 = MixableDto.value(mockMix);
        final dto2 = MixableDto.value(mockMix);
      ...
      test('same tokens are equal', () {
        ...
        final dto1 = MixableDto.token(mockToken);
        final dto2 = MixableDto.token(mockToken);
      ...
      test('different tokens are not equal', () {
        ...
        final dto1 = MixableDto.token(mockToken1);
        final dto2 = MixableDto.token(mockToken2);
    ...
    group('Edge cases', () {
      ...
      test('merging multiple times maintains consistency', () {
        ...
        final dto2 = MixableDto.value(mockMix2);
        final dto3 = MixableDto.value(mockMix3);
        final dto4 = MixableDto.value(mockMix4);
