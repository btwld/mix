  ...
  group('Decoration Image Utilities', () {
    group('DecorationImageUtility', () {
      ...
      group('Property Utilities', () {
        ...
        test('provider.network() creates decoration image with network image', () {
          final attr = utility.provider.network('https://example.com/image.png');
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });
        ...
        test('provider.memory() creates decoration image with memory image', () {
        ...
        test('fit.scaleDown() creates decoration image with scale down fit', () {
        ...
        test('alignment.center() creates decoration image with center alignment', () {
          final attr = utility.alignment.center();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('alignment.topLeft() creates decoration image with top left alignment', () {
          final attr = utility.alignment.topLeft();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('alignment.bottomRight() creates decoration image with bottom right alignment', () {
        ...
        test('centerSlice.zero() creates decoration image with zero center slice', () {
          final attr = utility.centerSlice.zero();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('centerSlice.largest() creates decoration image with largest center slice', () {
          final attr = utility.centerSlice.largest();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('centerSlice.fromLTRB() creates decoration image with LTRB center slice', () {
          final attr = utility.centerSlice.fromLTRB(5, 5, 15, 15);
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('centerSlice.fromLTWH() creates decoration image with LTWH center slice', () {
        ...
        test('filterQuality() creates decoration image with filter quality', () {
          final attr = utility.filterQuality(FilterQuality.high);
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('filterQuality.none() creates decoration image with no filter quality', () {
          final attr = utility.filterQuality.none();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('filterQuality.low() creates decoration image with low filter quality', () {
          final attr = utility.filterQuality.low();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('filterQuality.medium() creates decoration image with medium filter quality', () {
          final attr = utility.filterQuality.medium();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('filterQuality.high() creates decoration image with high filter quality', () {
        ...
        test('invertColors.on() creates decoration image with invert colors on', () {
          final attr = utility.invertColors.on();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('invertColors.off() creates decoration image with invert colors off', () {
        ...
        test('isAntiAlias.on() creates decoration image with anti alias on', () {
          final attr = utility.isAntiAlias.on();
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });

        test('isAntiAlias.off() creates decoration image with anti alias off', () {
      ...
      group('Edge Cases', () {
        ...
        test('handles asset path with special characters', () {
          final attr = utility.provider.asset('assets/image-with_special.chars.png');
          expect(attr.value, isA<MixProp<DecorationImage>>());
        });
      });
