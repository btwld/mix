  ...
  group('Text Height Behavior Utilities', () {
    group('TextHeightBehaviorUtility', () {
      ...
      group('Property Utilities', () {
        test('heightToFirstAscent() creates text height behavior with height to first ascent', () {
          final attr = utility.heightToFirstAscent(true);
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('heightToFirstAscent.on() creates text height behavior with height to first ascent on', () {
          final attr = utility.heightToFirstAscent.on();
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('heightToFirstAscent.off() creates text height behavior with height to first ascent off', () {
          final attr = utility.heightToFirstAscent.off();
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('heightToLastDescent() creates text height behavior with height to last descent', () {
          final attr = utility.heightToLastDescent(false);
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('heightToLastDescent.on() creates text height behavior with height to last descent on', () {
          final attr = utility.heightToLastDescent.on();
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('heightToLastDescent.off() creates text height behavior with height to last descent off', () {
          final attr = utility.heightToLastDescent.off();
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('leadingDistribution() creates text height behavior with leading distribution', () {
          final attr = utility.leadingDistribution(TextLeadingDistribution.proportional);
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('leadingDistribution.proportional() creates text height behavior with proportional leading distribution', () {
          final attr = utility.leadingDistribution.proportional();
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });

        test('leadingDistribution.even() creates text height behavior with even leading distribution', () {
      ...
      group('Leading Distribution Variations', () {
        test('creates text height behavior with proportional leading distribution', () {
          final attr = utility.leadingDistribution.proportional();
          expect(attr.value, isA<MixProp<TextHeightBehavior>>());
        });
      ...
      group('Height Application Combinations', () {
        test('creates text height behavior with both height applications enabled', () {
          final textHeightBehaviorMix = TextHeightBehaviorMix.only(
            applyHeightToFirstAscent: true,
            applyHeightToLastDescent: true,
          );
        ...
        test('creates text height behavior with both height applications disabled', () {
          final textHeightBehaviorMix = TextHeightBehaviorMix.only(
            applyHeightToFirstAscent: false,
            applyHeightToLastDescent: false,
          );
        ...
        test('creates text height behavior with only first ascent height enabled', () {
          final textHeightBehaviorMix = TextHeightBehaviorMix.only(
            applyHeightToFirstAscent: true,
            applyHeightToLastDescent: false,
          );
        ...
        test('creates text height behavior with only last descent height enabled', () {
          final textHeightBehaviorMix = TextHeightBehaviorMix.only(
            applyHeightToFirstAscent: false,
            applyHeightToLastDescent: true,
          );
      ...
      group('Edge Cases', () {
        ...
        test('creates text height behavior with only height to first ascent', () {
        ...
        test('creates text height behavior with only height to last descent', () {
