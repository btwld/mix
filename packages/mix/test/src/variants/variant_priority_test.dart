...
void main() {
  group('VariantPriority', () {
    ...
    test('NamedVariant has normal priority through VariantAttribute', () {
      ...
      const attribute = VariantSpecAttribute(
        variant,
        MockIntScalarAttribute(1),
      );
    });

    test('ContextVariant has normal priority by default', () {
      final variant = MockContextVariant();
      final attribute = VariantSpecAttribute(
        variant,
        MockIntScalarAttribute(1),
      );
    });
    ...
    test(
      'MultiVariant gets high priority if any variant has high priority',
      () {
    ...
    test(
      'MultiVariant gets normal priority if all variants have normal priority',
      () {
    ...
    test('MultiVariant priority works with OR operation', () {
      final highPriorityVariant = MockHighPriorityContextVariant();
      const normalVariant = NamedVariant('normal');
      final multiVariant = MultiVariant.or([
        normalVariant,
        highPriorityVariant,
      ]);
      final attribute = VariantSpecAttribute(
