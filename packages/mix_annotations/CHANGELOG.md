## 2.1.0
 ...
 - **BREAKING**: Removed `MixWidget.widgetBuilder` and the previous
   spec-side rendering annotation. `@MixWidget` rendering is now declared by
   the styler's concrete `call()` method.
 - **BREAKING**: Removed the unused styler call-generation flags. Styler
   `call()` methods are user-authored, not generated by `@MixableStyler`.
