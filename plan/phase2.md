...
| 2026-07-02 | Dummy drift demo re-run and recorded by Phase 2.5. | Temporary `Phase25ProbeStyler` source made `dart run tool/inventory_check.dart --check` fail with missing entries for `$animation`, `$modifier`, `$phase25Probe`, and `$variants`; the probe file was removed and the check returned green. |
