...
| 2026-07-02 | Cross-phase review follow-ups → phase 2.5. | Findings X3/X4/X8 (findings.md Addendum) found doc/behavior conflicts after closeout — lenient warning paths vs spec, `unsupported_value` naming, null-`v`/`validate()` diagnostics. Owned by `plan/phase2.5.md`; R1.4's "no code/doc conflict" box is re-verified there. Phase stays Completed. |
| 2026-07-02 | R1.4 re-verified by Phase 2.5. | Phase 2.5 restored WIRE_CONTRACT/code/test lockstep for original lenient warning paths, `unsupported_encode_value`, `v: null`, and validate-vs-decode transition warnings. |
