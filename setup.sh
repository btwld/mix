...
# Create an env file that can be sourced
ENV_FILE=".flutter_env"
{
  echo "# Auto-generated by setup.sh - source this file to set up PATH"
  echo "export PATH=\"$FVM_BIN_DIR:$PUB_CACHE_BIN:\$PATH\""
  if [ -n "$FLUTTER_SDK_BIN" ]; then
...
log "Setup complete."
log ""
log "To use flutter, melos, and other commands directly, run:"
log "  source $ENV_FILE"
log ""
log "Or add to your shell profile (~/.bashrc, ~/.zshrc):"
log "  export PATH=\"$FVM_BIN_DIR:$PUB_CACHE_BIN:\$PATH\""
if [ -n "$FLUTTER_SDK_BIN" ]; then
  log "  export PATH=\"$FLUTTER_SDK_BIN:\$PATH\""
fi
